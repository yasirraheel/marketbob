...
class ProcessCancelledSale
{
    public function handle(SaleCancelled $event)
    {
        ...
        $purchaseStatement->title = translate('[Cancellation] Purchase #:id (:item_name)', [
            'id' => $purchase->id,
            'item_name' => $item->name,
        ]);
        ...
        if ($buyerTax) {
            ...
            $buyerTaxStatement->title = translate('[Cancellation] :tax_name (:tax_rate%) Purchase #:id (:item_name)', [
                'id' => $purchase->id,
                'item_name' => $item->name,
        ...
        if ($authorTax) {
            ...
            $authorTaxStatement->title = translate('[Cancellation] :tax_name (:tax_rate%) Sale #:id (:item_name)', [
                'id' => $sale->id,
                'item_name' => $item->name,
        ...
        $saleStatement->title = translate('[Cancellation] Sale #:id (:item_name)', ['id' => $sale->id, 'item_name' => $item->name]);
        ...
        if ($referralEarning) {
            ...
            $referralStatement->title = translate('[Cancellation] Referral Earnings #:id', ['id' => $referralEarning->id]);
            ...
            $referralEarning->status = ReferralEarning::STATUS_CANCELLED;
