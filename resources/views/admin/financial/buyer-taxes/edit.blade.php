...
@section('title', translate('Edit Buyer Tax'))
@section('back', route('admin.financial.buyer-taxes.index'))
@section('container', 'container-max-lg')
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.financial.buyer-taxes.update', $buyerTax->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row g-3 row-cols-1">
                    <div class="col">
                        ...
                        <input type="text" name="name" class="form-control" value="{{ $buyerTax->name }}" required />
                    </div>
                    <div class="col">
                        <label class="form-label">{{ translate('Rate') }} </label>
                        <div class="input-group">
                            <input type="number" name="rate" class="form-control" value="{{ $buyerTax->rate }}"
                    ...
                    <div class="col">
                        ...
                        <select name="countries[]" class="form-select selectpicker" multiple data-live-search="true"
                            ...
                            @foreach (countries() as $countryCode => $countryName)
                                <option value="{{ $countryCode }}" @selected(in_array($countryCode, $buyerTax->countries))>
                                    {{ $countryName }}
                                </option>
                            @endforeach
                        </select>
