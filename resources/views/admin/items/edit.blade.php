...
@section('title', translate('Edit Item') . ' - ' . $item->name)
@section('content')
    <div class="mb-4">
        <a href="{{ route('admin.items.show', $item->id) }}" class="btn btn-secondary btn-sm">
            <i class="fa fa-arrow-left me-2"></i>{{ translate('Back to Item') }}
        </a>
    </div>
    <form action="{{ route('admin.items.update', $item->id) }}" method="POST">
            ...
            <div class="card-v-body p-4">
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" class="ckeditor">{{ old('description', $item->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-4 mb-3">
                    ...
                    <div class="col-lg-12">
                        ...
                        <select class="form-select form-select-md" disabled>
                            ...
                            @foreach ($category->subCategories as $subCategory)
                                <option value="{{ $subCategory->slug }}" @selected($item->subCategory && $item->subCategory->id == $subCategory->id)>
                                    {{ $subCategory->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @if ($category->categoryOptions->count() > 0)
                        @foreach ($category->categoryOptions as $categoryOption)
                            <div class="col-lg-12">
                                ...
                                @endphp
                                <label class="form-label">{{ $categoryOptionName }} @if($categoryOption->isRequired())<span class="text-danger">*</span>@endif</label>
                                <select
                                        ...
                                        @php
                                            ...
                                            if (isset($item['options'][$categoryOptionName])) {
                                                $selected = $categoryOption->isMultiple()
                                                    ? in_array($option, $item['options'][$categoryOptionName])
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="version" class="form-control form-control-md" 
                            placeholder="e.g., 1.0.0" value="{{ old('version', $item->version) }}">
                    </div>
                    <div class="col-12">
                        ...
                        <input type="url" name="demo_link" class="form-control form-control-md" 
                            placeholder="https://..." value="{{ old('demo_link', $item->demo_link) }}">
                    </div>
                    <div class="col-12">
                        <label class="form-label">{{ translate('Tags') }} <span class="text-danger">*</span></label>
                        <input id="item-tags" type="text" name="tags" value="{{ old('tags', $item->tags) }}" required>
                        <div class="form-text">
                    ...
                    <div class="col-12">
                        ...
                        <div id="features-container">
                            @php
                                $features = old('features', json_decode($item->features ?? '[]', true) ?: []);
                            @endphp
                            @if(count($features) > 0)
        ...
        @if (@settings('item')->support_status)
            <div class="dashboard-card card-v p-0 mb-4">
                ...
                <div class="card-v-body p-4">
                    <p>{{ translate('Item will be supported?') }}</p>
                    <div>
                        <div class="btn-group" role="group">
                            <input type="radio" class="btn-check support-option" name="support" value="0"
                                id="support1" @checked(old('support', $item->is_supported) == '0')>
                            ...
                            <input type="radio" class="btn-check support-option" name="support" value="1"
                                id="support2" @checked(old('support', $item->is_supported) == '1')>
                            <label class="btn btn-outline-primary" for="support2">{{ translate('Yes') }}</label>
                        </div>
                    </div>
                    <div class="support-instructions mt-3 {{ old('support', $item->is_supported) == '1' ? '' : 'd-none' }}">
                        <label class="form-label">{{ translate('Instructions') }}</label>
                        <textarea name="support_instructions" class="form-control" rows="6">{{ old('support_instructions', $item->support_instructions) }}</textarea>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="card-v-body p-4">
                @if($item->hasDiscount())
                    <div class="alert alert-info">
                        ...
                        {{ translate('This item has an active discount. Pricing cannot be changed until the discount expires.') }}
                    </div>
                @endif
                <div class="row g-4 mb-3">
                ...
                @if(!$item->hasDiscount())
                    ...
                    <div class="row g-4 mb-3">
                        @php
                            ...
                            $currentPrices = json_decode($item->validity_prices, true) ?? [];
                        @endphp
                            ...
                            <div class="col-md-6 col-lg-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" name="validity_prices[{{ $period['months'] }}]" 
                                        class="form-control" 
                                        value="{{ old('validity_prices.' . $period['months'], $currentPrices[$period['months']] ?? '') }}"
                                        min="{{ @settings('item')->minimum_price }}" 
                                        max="{{ @settings('item')->maximum_price }}">
                                </div>
                                <div class="form-text">
                    ...
                    <div>
                        <div class="btn-group" role="group">
                            <input type="radio" class="btn-check free-item-option" name="free_item" value="0"
                                id="op1" @checked(old('free_item', $item->is_free) == '0')>
                    ...
                    <div class="mt-3 {{ old('free_item', $item->is_free) == '1' ? '' : 'd-none' }} purchasing-option">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <input type="radio" class="btn-check" name="purchasing_status" value="0"
                                id="opg2" @checked(old('purchasing_status', $item->purchasing_status) == '0')>
        ...
        <div class="dashboard-card card-v p-0 mb-3">
            ...
            <div class="card-v-body p-4">
                ...
                <select name="status" class="form-select form-select-md" required>
                    <option value="1" @selected(old('status', $item->status) == 1)>{{ translate('Pending') }}</option>
                    <option value="2" @selected(old('status', $item->status) == 2)>{{ translate('Soft Rejected') }}</option>
                    <option value="3" @selected(old('status', $item->status) == 3)>{{ translate('Hard Rejected') }}</option>
                    <option value="4" @selected(old('status', $item->status) == 4)>{{ translate('Resubmitted') }}</option>
                    <option value="5" @selected(old('status', $item->status) == 5)>{{ translate('Approved') }}</option>
                    <option value="6" @selected(old('status', $item->status) == 6)>{{ translate('Updated') }}</option>
                </select>
            </div>
        </div>
        <div class="dashboard-card card-v p-0 mb-3">
        ...
        <div class="dashboard-card card-v p-0">
            <div class="card-v-body p-4">
                <button class="btn btn-primary btn-md">{{ translate('Update Item') }}</button>
                <a href="{{ route('admin.items.show', $item->id) }}" class="btn btn-secondary btn-md">{{ translate('Cancel') }}</a>
            </div>
        </div>
    </form>
    @push('top_scripts')
        <script>
            ...
            const itemConfig = {!! json_encode([
                ...
                'load_files_route' => route('admin.items.files.load', [hash_encode($category->id), $item->author_id]),
            ]) !!};
        </script>
    @endpush
    @push('styles_libs')
    ...
    @push('scripts_libs')
        ...
        <script>
            $(function() {
                ...
                // Main file source switcher
                if ($('select[name="main_file_source"]').length) {
                    $('select[name="main_file_source"]').on('change', function() {
                        const val = $(this).val();
                        if (val == '0') {
                            $('.main-file-source-1').removeClass('d-none').prop('disabled', false);
                            $('.main-file-source-2').addClass('d-none').prop('disabled', true);
                        } else {
                            $('.main-file-source-1').addClass('d-none').prop('disabled', true);
                            $('.main-file-source-2').removeClass('d-none').prop('disabled', false);
                        }
                    }).trigger('change');
                }
            });
