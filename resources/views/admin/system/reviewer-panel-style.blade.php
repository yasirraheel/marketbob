...
@section('title', translate('Reviewer Panel Style'))
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.system.reviewer-panel-style') }}" method="POST">
        @csrf
        <div class="card mb-3">
            ...
            <div class="card-body">
                <div class="row g-3">
                    @foreach (@$settings->system_reviewer->colors as $key => $value)
                        <div class="col-lg-6 col-xl-4">
                            <label class="form-label"> {{ translate(ucfirst(str_replace('_', ' ', $key))) }}</label>
                            <div class="colorpicker">
                                <input type="text" name="system_reviewer[colors][{{ $key }}]"
