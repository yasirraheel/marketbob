...
@section('breadcrumbs', Breadcrumbs::render('workspace.settings.badges'))
@section('content')
    @include('themes.basic.workspace.settings.includes.tabs')
    <div class="dashboard-card card-v">
        <div class="form-section">
            <h5 class="mb-0">{{ translate('My Badges') }}</h5>
        </div>
        @if ($userBadges->count() > 0)
            <ul class="sortable-list custom-list-group list-group">
                @foreach ($userBadges as $userBadge)
                    <li class="list-group-item p-3" data-id="{{ $userBadge->id }}">
                                ...
                                <div class="row g-2 align-items-center">
                                    <div class="col-auto">
                                        <img src="{{ $userBadge->badge->getImageLink() }}"
                                            ...
                                            title="{{ $userBadge->badge->getFullTitle() }}" width="35px" height="35px">
                                    </div>
                                    <div class="col-auto">
    ...
    @push('top_scripts')
        <script>
            "use strict";
            const sortableRoute = "{{ route('workspace.settings.badges.sortable') }}";
        </script>
    @endpush
    @push('styles_libs')
