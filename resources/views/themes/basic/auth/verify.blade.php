...
@section('title', translate('Verify Your Email Address'))
@section('content')
    <div class="sign-box sign-box-lg">
        <div class="card-v">
            <div class="mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="170" height="170" viewBox="0 0 570 511.67482"
                    ...
                    <path
                        ...
                        transform="translate(-315 -194.16259)" fill="{{ $themeSettings->colors->primary_color }}" />
                    <path
            ...
            <div>
                <p class="h3 mb-3">{{ translate('Verify Your Email Address') }}</p>
                <p class="mb-3">
                    {{ translate("Please verify your email, simply click on the verification link that has been sent to your email address. If you haven't received the verification email, please check your spam or junk folder, or request a new verification email to be sent.") }}
                </p>
                <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                    @csrf
                    <button type="submit" class="btn btn-primary btn-md me-2">{{ translate('Resend') }}</button>
                </form>
                <button class="btn btn-outline-primary btn-md" data-bs-toggle="modal"
                    data-bs-target="#emailModal">{{ translate('Change Email') }}</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-4">
                <div class="modal-header border-0 p-0 mb-3">
                    <h5 class="modal-title" id="createFolderModalLabel">{{ translate('Email address') }}</h5>
                    ...
                    <div class="modal-body p-0">
                        <div class="mb-3">
                            <input type="email" name="email" id="email" class="form-control form-control-md"
                                placeholder="{{ translate('Email address') }}" value="{{ authUser()->email }}" required>
                        </div>
                        <div class="row justify-content-center g-3">
                            ...
                            <div class="col-12 col-lg">
                                <button type="submit"
                                    class="btn btn-primary btn-md w-100 me-2">{{ translate('Save') }}</button>
                            </div>
                        </div>
                    </div>
                </form>
